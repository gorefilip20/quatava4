...
import { getOrdersByUserId } from "@b/api/(ext)/ecosystem/utils/scylla/queries";
import { fromBigInt } from "@b/api/(ext)/ecosystem/utils/blockchain";
import { logError } from "@b/utils/logger";

export const metadata = {};

class EcosystemOrderHandler {
  private static instance: EcosystemOrderHandler;
  ...
  public static getInstance(): EcosystemOrderHandler {
    if (!EcosystemOrderHandler.instance) {
      EcosystemOrderHandler.instance = new EcosystemOrderHandler();
    }
    return EcosystemOrderHandler.instance;
  }
  ...
  private flushOrders() {
    if (Object.keys(this.trackedOrders).length > 0) {
      const route = "/api/ecosystem/order";
  ...
  private convertBigIntFields(order: any): any {
    return {
      ...order,
  ...
  private async fetchAndUpdateOrders(userId: string) {
    try {
      ...
      logError("ecosystem", error, __filename);
    }
  }

  public async fetchOrdersForUser(userId: string) {
    while (
      hasClients("/api/ecosystem/order") &&
...
export default async (data: Handler, message: any) => {
  const handler = EcosystemOrderHandler.getInstance();
