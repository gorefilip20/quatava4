  ...
  try {
    ...
    const result = await sequelize.transaction(async (dbTransaction) => {
      ...
      if (mappedStatus === 'COMPLETED') {
        ...
        if (gateway) {
          const percentageFee = Number(gateway.getPercentageFee(paymentDetails.currencyCode))
          const fixedFee = Number(gateway.getFixedFee(paymentDetails.currencyCode))
          const totalFee = (Number(paymentAmount) * percentageFee / 100) + fixedFee
